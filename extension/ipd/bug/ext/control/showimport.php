    ...
    public function showImport($productID, $branch = 0, $pagerID = 1, $insert = '')
    {
        ...
        if($_POST)
        {
            ...
            if($this->post->insert)
            {
                ...
                $this->config->bug->form->batchCreate['injection']   = array('required' => false, 'type' => 'string', 'default' => '');
                $this->config->bug->form->batchCreate['identify']    = array('required' => false, 'type' => 'string', 'default' => '');
                $this->config->bug->form->batchCreate['assignedTo']  = array('required' => false, 'type' => 'string', 'default' => '');
        ...
        $branchModules = $this->loadModel('tree')->getOptionMenu($productID, 'bug', 0, empty($branches) ? array(0) : array_keys($branches));
