...
class baseRouter
{
     ...
     * @return void
     */
    public function __construct($appName = 'demo', $appRoot = '', $mode = 'running')
    {
   ...
   /**
    ...
    public function setVision()
    {
        ...
        if($vision and strpos((string) $this->config->visions, ",{$vision},") === false) $vision = $defaultVision;
    ...
    {
        set_error_handler(\Closure::fromCallable([$this, 'saveError']));
   ...
   /**
    ...
    public function setClientDevice()
    {
        ...
        if(empty($this->cookie->device) || strpos('mobile,desktop', (string) $this->cookie->device) === false)
        {
            $mobile = new mobile();
    ...
    {
        if(strncmp($_SERVER['REQUEST_URI'], '/data/upload', strlen('/data/upload')) === 0 && !is_file($this->wwwRoot . $_SERVER['REQUEST_URI']))
        {
    ...
    public function parsePathInfo()
    {
        ...
        if(!empty($pathInfo))
        {
            ...
            if($dotPos)
            {
                ...
                if(strpos((string) $this->config->views, ',' . $this->viewType . ',') === false)
                {
    ...
    public function getPathInfo()
    {
        ...
        elseif(isset($this->uri))
        {
            ...
            if($subpath != '' and $subpath != '/' and strncmp($value, $subpath, strlen($subpath)) === 0) $value = substr($value, strlen($subpath));
        }
        else
        {
        ...
        if(strpos((string) $value, (string) $_SERVER['SCRIPT_NAME']) !== false) $value = str_replace($_SERVER['SCRIPT_NAME'], '', (string) $value);
        if(strpos((string) $value, '?') === false) return trim((string) $value, '/');
     ...
     * @return string
     */
    public static function removePHPTAG($fileName)
    {
        $code = trim(file_get_contents($fileName));
        if(strncmp($code, '<?php', strlen('<?php')) === 0) $code = ltrim($code, '<?php');

        $code = ltrim($code);
        if(strncmp($code, 'declare(strict_types=1);', strlen('declare(strict_types=1);')) === 0) $code = ltrim($code, 'declare(strict_types=1);');
     ...
     * @return string
     */
    public static function extractAPIURL($fileName)
    {
        global $config;
    ...
    public function setRouteByPathInfo()
    {
        if(!empty($this->uri))
        {
             ...
             **/
            if(strpos($this->uri, (string) $this->config->requestFix) !== false)
            {
    ...
    {
        return strtolower((string)getenv('ZT_AUTO_UPGRADE')) == 'true';
    }
    ...
    {
        return strtolower((string)getenv('ZT_CHECK_SAFE_FILE')) == 'true';
    }
