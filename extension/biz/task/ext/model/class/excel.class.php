...
class excelTask extends taskModel
{
    ...
    public function createFromImport($tasks)
    {
        ...
        $tasksID   = array();
        $levelList = $this->session->levelList ? $this->session->levelList : array();
        foreach($tasks as $key => $taskData)
        {
            if(isset($taskData->estimate)) $taskData->estimate = round((float)$taskData->estimate, 2);
            ...
            if(!empty($levelList)) $this->session->set('levelList', $levelList);
        }
