...
class myProject extends project
{
    ...
    public function edit($projectID, $from = '', $programID = 0, $extra = '')
    {
        ...
        $project->model    = isset($output['model'])     ? $output['model']     : $project->model;
        $project->category = isset($output['category'])  ? $output['category']  : $project->category;
        $hasProduct        = isset($project->hasProduct) ? $project->hasProduct : 1;
        $workflowGroups    = $this->loadModel('workflowgroup')->getPairs('project', $project->model, $hasProduct, 'normal', '0', !empty($project->category) ? $project->category : 'ipd');
        $workflowGroup     = $project->workflowGroup;
        if(!isset($workflowGroups[$workflowGroup]) && $workflowGroup)
        {
            $group = $this->workflowgroup->getByID($workflowGroup);
            if($group->objectID)
            {
                if($group && $group->projectModel == $project->model) $workflowGroups[$workflowGroup] = $group->name;
                $this->view->singleProjectFlow = true;
            }
        }
        ...
        parent::edit($projectID, $from, $programID, $extra);
    }
}
