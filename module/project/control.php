...
class project extends control
{
     ...
     * @return void
     */
    public function ajaxGetDropMenu($projectID, $module, $method, $extra = '', $useLink = 1)
    {
     ...
     * @return void
     */
    public function ajaxGetOldDropMenu($projectID, $module, $method, $extra = '')
    {
        /* Set cookie for show all project. */
        $_COOKIE['showClosed'] = 1;
     ...
     * @return void
     */
    public function ajaxGetRemoveMemberTips($projectID, $account)
    {
     ...
     * @return void
     */
    public function ajaxGetProjectFormInfo($objectType, $objectID, $selectedProgramID)
    {
     ...
     * @return void
     */
    public function createGroup($projectID)
    {
        $this->loadModel('group');
        if(!empty($_POST))
        {
     ...
     * @return void
     */
    public function dynamic($projectID = 0, $type = 'today', $param = '', $recTotal = 0, $date = 0, $direction = 'next')
    {
     ...
     * @return void
     */
    public function execution($status = 'undone', $projectID = 0, $orderBy = 'order_asc', $productID = 0, $recTotal = 0, $recPerPage = 100, $pageID = 1, $queryID = 0)
    {
     ...
     * @return void
     */
    public function managePriv($projectID, $groupID = 0)
    {
        $this->loadModel('group');
     ...
     * @return void
     */
    public function unlinkMember($projectID, $userID, $removeExecution = 'no')
    {
    ...
    public function suspend($projectID)
    {
        ...
        if(!empty($_POST))
        {
            ...
            $comment = strip_tags($this->post->comment, $this->config->allowedTags !== null && is_array($this->config->allowedTags) ? '<' . implode('><', $this->config->allowedTags) . '>' : $this->config->allowedTags);
    ...
    public function close($projectID)
    {
        ...
        if(!empty($_POST))
        {
            ...
            $comment = strip_tags((string)$this->post->comment, $this->config->allowedTags !== null && is_array($this->config->allowedTags) ? '<' . implode('><', $this->config->allowedTags) . '>' : $this->config->allowedTags);
   ...
   /**
     ...
     * @return void
     */
    public function ajaxGetExecutions($projectID, $mode = '', $type = 'all', $pageType = '')
    {
        $executions = array();
     ...
     * @return void
     */
    public function ajaxSwitcherMenu($projectID, $productID, $currentMethod = '')
    {
     ...
     * @return void
     */
    public function ajaxGetWorkflowGroups($model, $hasProduct, $category)
    {
