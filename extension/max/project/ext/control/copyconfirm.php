...
class project extends control
{
    ...
    public function copyConfirm($copyProjectID, $products = '', $copyFrom = '')
    {
        ...
        if(strtolower($this->server->request_method) == "post" || !$project->multiple)
        {
            $copyProject = json_decode($this->cookie->copyData, true);
            if(!$project->multiple) $_POST = $copyProject;
            ...
            $projectData['begin']     = isset($copyProject['begin']) ? $copyProject['begin'] : '';
            $projectData['end']       = isset($copyProject['end']) ? $copyProject['end'] : '';
            ...
            $projectID = $this->project->saveCopyProject($copyProjectID, $model, $execution, $copyFrom);
            if(dao::isError()) return $this->send(array('result' => 'fail', 'message' => dao::getError()));
            ...
            if(!$project->multiple) return $this->send(array('result' => 'success', 'load' => inlink('create', "model=$model&programID={$project->parent}&copyProjectID={$project->id}&extra=showTips=1,project={$projectID}")));
        ...
        if(in_array($project->model, array('waterfall', 'waterfallplus', 'ipd')))
        {
            ...
            if($productIdList)
            {
                ...
                {
                    if(empty($executionGroupIdList[$product->id])) continue;

                    $productPairs[$product->id]    = $product->name;
                    $executionIdList[$product->id] = $executionGroupIdList[$product->id];
                }
            }
