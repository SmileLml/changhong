                ...
                <div class='target-form-group'>
                  ...
                  <div class='options'>
                    <?php foreach(array_keys($forms) as $form):?>
                      <div class='option'>
                        <input type='radio' name='targetForm' value='<?php echo "$name.$form";?>' <?php echo ("$name.$form" == $prompt->targetForm and empty($prompt->triggerControl)) ? 'checked' : '';?> <?php echo !empty($prompt->triggerControl) ? "disabled='disabled'" : '';?>>
    ...
    success: function(response, status)
    {
      ...
      if(message)
      {
        ...
        if($targetSubmitBtn.length)
        {
          ...
          setTimeout(function() {$targetSubmitBtn.popover('destroy');}, 3000);
        }
      }
    },
