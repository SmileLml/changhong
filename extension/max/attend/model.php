...
class attendModel extends model
{
    ...
    public function getGroupByAccount($startDate = '', $endDate = '')
    {
        ...
        $users = $this->loadModel('user')->getDeptPairs('noclosed,noempty,nodeleted,noforbidden');
        $attends = $this->dao->select('*')->from(TABLE_ATTEND)
            ->where(1)
            ->beginIf($startDate != '')->andWhere('`date`')->ge($startDate)->fi()
            ->beginIf($endDate != '')->andWhere('`date`')->le($endDate)->fi()
    ...
    {
        $this->processStatus();
        $users = $this->loadModel('user')->getDeptPairs('noclosed,noempty,nodeleted,noforbidden', $deptID);
        ...
        {
            if($dept == 0) continue;
            $deptUsers = $this->user->getDeptPairs('noclosed,noempty,nodeleted,noforbidden', $dept);
    ...
    public function getDetailAttends($date = '', $account = '', $deptID = 0)
    {
        ...
        $deptList = arrayUnion(array(''), $this->loadModel('dept')->getDeptPairs());
    ...
    public function getAllMonth($type = '')
    {
        if($type == 'department')
        {
            $deptList = $this->loadModel('dept')->getDeptManagedByMe($this->app->user->account);
    ...
    public function getNotice()
    {
        ...
        $link    = helper::createLink('attend', 'personal');
        $misc    = "class='app-btn alert-link' data-id=''";
        $notice  = '';
...
EOT;
    ...
    public function computeLeaveStat($stat, $year, $month)
    {
        $startDate = $month ? "{$year}-{$month}-01" : "$year-01-01";
        $endDate   = $month ? date('Y-m-d', strtotime("$startDate +1 month -1 day")) : "$year-12-31";
        $leaves    = $this->loadModel('leave')->getList($type = 'company', $year, $month, $account = '', $dept = '', $status = 'pass', $orderBy = 'begin, start');
    ...
    public function computeLieuStat($stat, $year, $month)
    {
        $startDate = $month ? "{$year}-{$month}-01" : "$year-01-01";
        $endDate   = $month ? date('Y-m-d', strtotime("$startDate +1 month -1 day")) : "$year-12-31";
        $lieus     = $this->loadModel('lieu')->getList($type = 'company', $year, $month, $account = '', $dept = '', $status = 'pass', $orderBy = 'begin');
    ...
    public function computeMakeupStat($stat, $year, $month)
    {
        $startDate = $month ? "{$year}-{$month}-01" : "$year-01-01";
        $endDate   = $month ? date('Y-m-d', strtotime("$startDate +1 month -1 day")) : "$year-12-31";
        $makeups   = $this->loadModel('makeup')->getList($type = 'company', $year, $month, $account = '', $dept = '', $status = 'pass', $orderBy = 'begin, start');
    ...
    public function computeOvertimeStat($stat, $year, $month)
    {
        $startDate = $month ? "{$year}-{$month}-01" : "$year-01-01";
        $endDate   = $month ? date('Y-m-d', strtotime("$startDate +1 month -1 day")) : "$year-12-31";
        $allLieus  = $this->loadModel('lieu',     '')->getList($type = 'company', '', '', '', '', 'pass');
        $overtimes = $this->loadModel('overtime')->getList($type = 'company', $year, $month, $account = '', $dept = '', $status = 'pass', $orderBy = 'begin, start');
    ...
    public function computeTripStat($stat, $year, $month)
    {
        $startDate = $month ? "{$year}-{$month}-01" : "$year-01-01";
        $endDate   = $month ? date('Y-m-d', strtotime("$startDate +1 month -1 day")) : "$year-12-31";
        $trips     = $this->loadModel('trip')->getList($type = '', $year, $month, $account = '', $dept = '', $orderBy = 'begin, start');
     ...
     * @return bool
     */
public function processStatus($module = null, $record = null)
    {
        $attends = $this->dao->select('*')->from(TABLE_ATTEND)
            ->where('status')->eq('')
    ...
    public function computeStatus($attend)
    {
        ...
        /* 'rest': rest day. */
        if($this->isRestDay($attend->date)) $status = $attend->reason == 'overtime' ? 'overtime' : 'rest';
    ...
    public function processHours($attendList, $startDate)
    {
        ...
        $leaveList    = $this->loadModel('leave',    '')->getList('',        $year, $month, '', '', 'pass');
        $lieuList     = $this->loadModel('lieu',     '')->getList('',        $year, $month, '', '', 'pass');
        $makeupList   = $this->loadModel('makeup',   '')->getList('',        $year, $month, '', '', 'pass');
        $overtimeList = $this->loadModel('overtime')->getList('',        $year, $month, '', '', 'pass');
        $tripList     = $this->loadModel('trip',     '')->getList('trip',    $year, $month);
        $egressList   = $this->loadModel('trip',     '')->getList('egreess', $year, $month);
    ...
    public function setManager()
    {
        ...
        foreach($deptList as $id => $dept)
        {
            ...
            $this->dao->update(TABLE_CATEGORY)->set('manager')->eq($dept)->where('id')->eq($id)->andWhere('type')->eq('dept')->exec();
        }
        return !dao::isError();
    }
    ...
    public function savePersonalSettings()
    {
        ...
        if($beginDates) $this->loadModel('setting')->setItems('system.attend.beginDate', $beginDates);
