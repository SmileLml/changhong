...
class ticketModel extends model
{
    ...
    public function create($extras)
    {
        ...
        $ticket = fixer::input('post')
            ...
            ->setIF($this->post->estimate, 'estimate', round((float)$this->post->estimate, 2))
            ->trim('title')
            ->join('mailto', ',')
            ->join('openedBuild', ',')
    ...
    public function finish($ticketID)
    {
        ...
        $ticket = fixer::input('post')
            ...
            ->setIF($this->post->consumed, 'consumed', round((float)$this->post->consumed, 2))
    ...
    public function batchCreate($tickets = array(), $productID = 0)
    {
        ...
        foreach($tickets as $rowIndex => $ticket)
        {
            ...
            if(isset($ticket->estimate)) $ticket->estimate = round((float)$ticket->estimate, 2);
    ...
    public function batchFinish()
    {
        ...
        foreach($tickets as $ticketID => $ticket)
        {
            ...
            if(isset($ticket->consumed)) $ticket->consumed = round((float)$ticket->consumed, 2);
