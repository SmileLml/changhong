...
(
    ...
    (
        set::isForm(true),
        tableData(setClass('mt-5'), set::title($lang->story->legendBasicInfo), !$product->shadow ? item
        (
            ...
            (
                ...
                (
                    ...
                    $product->type != 'normal' ? picker(setID('branch'), set::width('100px'), set::name('branch'), set::items($fields['branch']['options']), set::value($fields['branch']['default'])) : null
                )
            )
        ) : null, $story->parent > 0 && $product->type != 'normal' ? item
        (
        ...
        ) : picker(setID('grade'), set::name('grade'), set::required(true), set::items($fields['grade']['options']), set::value($story->grade), set::hidden(true)), $showPlan ? item
        (
            set::trClass(zget($fields['plan'], 'className', '')),
            set::name($lang->story->plan),
            inputGroup
            (
                span(setID('planIdBox'), picker(set::name($multiplePlan ? 'plan[]' : 'plan'), set::items($fields['plan']['options']), set::value($fields['plan']['default']), set::multiple($multiplePlan))),
        ...
        tableData(set::title($lang->story->legendLifeTime), item
        (
        ...
        $story->status == 'closed' ? tableData(set::title($lang->story->legendMisc), item
        (
