        ...
        foreach($this->groupTasks as $groupKey => $tasks)
        {
            ...
            foreach($tasks as $taskKey => $task)
            {
                if($this->orderBy == 'story')
                {
                    if(!$task->isParent)
                    {
                        $groupEstimate += (float)$task->rawEstimate;
                        $groupConsumed += (float)$task->rawConsumed;
                        if($task->status != $this->lang->task->statusList['cancel'] && $task->status != $this->lang->task->statusList['closed']) $groupLeft += (float)$task->rawLeft;
                    }
                }
                else
                {
                    if(!$task->isParent)
                    {
                        ...
                        if($this->orderBy == 'status' || ($task->status != $this->lang->task->statusList['cancel'] && $task->status != $this->lang->task->statusList['closed'])) $groupLeft += (float)$task->rawLeft;
                    }
                }
