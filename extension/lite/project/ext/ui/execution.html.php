...
$fnGetKanbanTeams = function($kanbanID) use ($memberGroup, $users, $usersAvatar)
{
    ...
    foreach($teams as $member)
    {
        if($count > 2) break;
        $count ++;
        $teamElements[] = div(set::title(zget($users, $member->account)), avatar(set::src(zget($usersAvatar, $member->account)), set::text(zget($member, 'realname', '')), set::size('sm'), set::className('mr-1')));
    }
    if(count($teams) > 4) $teamElements[] = span('… ');
    if(count($teams) > 3)
    {
        $lastMember = end($teams);
        $teamElements[] = div(set::title(zget($users, $lastMember->account)), avatar(set::src(zget($usersAvatar, $lastMember->account)), set::text(zget($lastMember, 'realname', '')), set::size('sm')));
    }
    return $teamElements;
};
