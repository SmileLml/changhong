...
class reviewModel extends model
{
    ...
    public function create($projectID = 0, $reviewRange = 'all', $checkedItem = '')
    {
        ...
        {
            $templateID = $this->dao->select('id')->from(TABLE_DOC)
        ...
        $flowID = $this->loadModel('approvalflow')->getFlowIDByObject($projectID, $data->object);
        $nodes  = $this->loadModel('approval')->getNodesToConfirm($flowID, 0, $projectID, $data->product);
        foreach($nodes as $node)
        {
            if(!isset($node['required']) || $node['required'] == 'no') continue;

            $nodeID = $node['id'];
            if(empty(array_filter($_POST['reviewer'][$nodeID]))) dao::$errors["reviewer{$nodeID}"] = sprintf($this->lang->error->notempty, $this->lang->approval->reviewer);
        }
    ...
    public function update($reviewID)
    {
        ...
        $this->file->processFileDiffsForObject('review', $oldReview, $data);
    ...
    public function saveResult($reviewID, $type = 'review')
    {
        ...
        foreach($issueResult as $id => $result)
        {
            ...
            if(!empty($data->opinionDate[$id])) $issue->opinionDate = $data->opinionDate[$id];
