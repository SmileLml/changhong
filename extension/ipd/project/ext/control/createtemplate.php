...
class myProject extends project
{
    ...
    public function createTemplate($model = 'scrum', $programID = 0, $copyProjectID = 0, $extra = '', $pageType = 'base')
    {
        ...
        $pauseFlowProjects = $this->dao->select('t1.id')->from(TABLE_PROJECT)->alias('t1')
            ->leftJoin(TABLE_WORKFLOWGROUP)->alias('t2')->on('t1.workflowGroup=t2.id')
            ->where('t2.status')->eq('pause')
            ->fetchPairs();

        $copyProjectList = $this->project->getPairs(false, 'nokanban,haspriv');
        $copyProjectList = array_diff_key($copyProjectList, $pauseFlowProjects);
        ...
        $this->view->copyProjectList                = $copyProjectList;
        ...
        $this->view->workflowGroupPairs             = $this->loadModel('workflowgroup')->getAllPairs();
