...
<style>
  ...
  .input-field {width: 80px; cursor: text;}
  .percent-input {width: 30px; text-align: center;}
  .input-wrapper {display: flex; align-items: center; margin: 0 10px;}
  .input-wrapper .input-field {border-radius: 4px 0 0 4px; border-right: none;}
  .input-wrapper .percent-input {border-radius: 0 4px 4px 0; margin-left: -1px;}
...
class SelectedDataSorter extends HTMLDivElement
{
  ...
  render()
  {
    const existingList  = this.querySelector('ol');
    const existingItems = existingList ? Array.from(existingList.querySelectorAll('li')) : [];
    ...
    props.forEach(prop =>
    {
      let existingItem = existingItems.find(item => item.getAttribute('prop') === prop);
      if(existingItem)
			{
        listView.appendChild(existingItem);
      }
			else
			{
        ...
        percentInput.classList.add('form-control', 'input-sm', 'percent-input');
        percentInput.value = '%';
        ...
        inputField.addEventListener('focus', (e) => {
          e.target.select();
          validateTotalSumWithSmartInput(e.target);
        });
...
{
	return validateTotalSumWithSmartInput(null);
}
    ...
    <div class='center-wrapper'>
      <div class='center-content'>
        ...
        <div style='display: flex; flex-grow: 1; flex-direction: column-reverse;'>
          ...
          <?php echo html::hidden('datasourceweight', isset($prompt->datasourceweight) ? $prompt->datasourceweight : 0.00);?>
...
function validateForm()
{
  ...
  if(pass)
  {
    ...
    {
      $.zui.messager.danger('<?php echo $lang->ai->dataSourceWeightError;?>');
      pass = false;
    }
  }
  ...
  if(pass)
  {
    const selectedDataSorter = document.querySelector('#selected-data-sorter');
    const weightValues = [];
    const listItems = selectedDataSorter.querySelectorAll('.list-group-item');
    listItems.forEach(item => {
      const inputField = item.querySelector('input[type="text"].input-field');
      if(inputField)
      {
        const weight = parseFloat(inputField.value) || 0;
        weightValues.push(weight.toFixed(2));
      }
    });
    let weightField = document.querySelector('input[name="datasourceweight"]');
    if(!weightField)
    {
      weightField = document.createElement('input');
      weightField.type = 'hidden';
      weightField.name = 'datasourceweight';
      document.querySelector('#mainForm').appendChild(weightField);
    }
    weightField.value = weightValues.join(',');
