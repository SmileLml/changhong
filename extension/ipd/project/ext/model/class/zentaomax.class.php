...
class zentaomaxProject extends projectModel
{
    ...
    public function saveCopyProject($copyProjectID, $model = 'scrum', $executions = array(), $copyFrom = '')
    {
        ...
        if(in_array($model, array('waterfall', 'waterfallplus', 'ipd')) && !empty($executions->executionIDList))
        {
            ...
            {
                if(isset($executions->productIdList) && !in_array($productID, $executions->productIdList)) continue;
                list($executionList, $parentAcl) = $this->setInsertExecutions($executions, $executionIdList, $model, $productID, $parentAcl, $copyFrom);

                $insertExecutions = arrayUnion($insertExecutions, $executionList);
        ...
        {
            list($insertExecutions) = $this->setInsertExecutions($executions, $executions->executionIDList, $model, 0, '', $copyFrom);
        ...
        /* 如果阶段不允许并行，则检查计划起止时间。*/
        if($copyProject->model == 'ipd' && empty($copyProject->parallel))
        {
            $prevEnd = '';
            $prevID  = 0;
            ...
            {
                if($execution->parent == $prevID) continue; // 子不跟父比
                $currentBegin = $execution->begin;

                if($prevEnd && $prevEnd > $currentBegin)
                {
                    dao::$errors['message'][] = $this->lang->programplan->error->outOfDate;
                    return false;
                }

                $prevEnd = $execution->end;
                $prevID  = $execution->executionID;
            }
        }
        ...
        if($copyFrom != 'project')
        {
            $extraFields = $this->loadModel('workflowfield')->getFieldPairs('project', '0');
            foreach($extraFields as $field => $fieldName)
            {
                if($field && !isset($project->{$field}) && !empty($copyProject->{$field})) $project->{$field} = $copyProject->{$field};
            }
        }
        ...
        /* 无产品项目将post过来的产品关联到模板中。 */
        if(empty($project->hasProduct) && !empty($projectPostData->rawdata->products))
        {
            $data = new stdClass();
            $data->product = current($projectPostData->rawdata->products);
        ...
        $productID = $this->dao->select('product')->from(TABLE_PROJECTPRODUCT)->where('project')->eq($projectID)->fetch('product');
        ...
        if(!$copyProject->multiple)
        {
            ...
            $insertModuleList = $this->copyExecutionTaskModule($copyExecutionID, $executionID);
            $insertTaskList   = $this->saveTask($copyProjectID, $projectID, $copyExecutionID, $executionID, $insertModuleList, $copyFrom);
            $insertExecutionList[$copyExecutionID] = $executionID;
        }
        else
        {
            list($insertExecutionList, $insertTaskList) = $this->saveExecutions($insertExecutions, $productID, $copyProjectID, $project, $projectID, $model, $copyFrom);
            if(dao::isError()) return false;
        }

        /* 复制项目的任务关系。 */
        $this->copyProjectTaskRelations($copyProjectID, $projectID, $insertExecutionList, $insertTaskList);

        /* 复制项目的审批设置。 */
        $this->copyProjectApprovalFlow($copyProjectID, $projectID);
    ...
    public function setInsertExecutions($executions = array(), $executionIdList = array(), $model = 'scrum', $productID = 0, $parentAcl = '', $copyFrom = '')
    {
        ...
        $nameList         = $isStage ? $executions->name[$productID] : $executions->name;
        $parentList       = $isStage ? $executions->parent[$productID] : $executions->parent;
        $PMList           = $isStage ? $executions->PM[$productID] : $executions->PM;
        $beginList        = $isStage ? $executions->begin[$productID] : $executions->begin;
        $endList          = $isStage ? $executions->end[$productID] : $executions->end;
        if(isset($executions->code)) $codeList = $isStage ? $executions->code[$productID] : $executions->code;
        ...
        $extraFields = $this->loadModel('workflowfield')->getFieldPairs('execution', '0');
        foreach($executionIdList as $executionID)
        {
            ...
            $insertExecution->PO            = $originExecutions[$executionID]->PO;
            $insertExecution->QD            = $originExecutions[$executionID]->QD;
            $insertExecution->RD            = $originExecutions[$executionID]->RD;
            $insertExecution->code          = !empty($codeList[$executionID]) ? $codeList[$executionID] : '';
            ...
            if($copyFrom != 'project')
            {
                foreach($extraFields as $field => $fieldName)
                {
                    if($field && !empty($originExecutions[$executionID]->{$field})) $insertExecution->{$field} = $originExecutions[$executionID]->{$field};
                }
            }

            if($isStage)
            {
                $insertExecution->attribute = $executions->attribute[$productID][$executionID];
                $insertExecution->product   = $productID;
                $insertExecution->percent   = $executions->percent[$productID][$executionID];
            ...
            {
                $insertExecution->lifetime = $executions->lifetime[$executionID];
                $insertExecution->days     = $executions->days[$executionID] ? $executions->days[$executionID] : 0;
                if(isset($this->config->setCode) and $this->config->setCode) $insertExecution->code = $executions->code[$executionID];
            }
    ...
    public function saveExecutions($insertExecutions = array(), $productID = 0, $copyProjectID = 0, $project = null, $projectID = 0, $model = 'scrum', $copyFrom = '')
    {
        ...
        foreach($insertExecutions as $insertExecution)
        {
            ...
            if(in_array($model, array('waterfall', 'waterfallplus', 'ipd')) and !empty($project->stageBy))
            {
                ...
                foreach($_POST['products'] as $projectProductID) $this->dao->insert(TABLE_PROJECTPRODUCT)->set('project')->eq($lastExecutionID)->set('product')->eq($projectProductID)->exec();
            }
        ...
        if($this->config->edition == 'ipd' && $model == 'ipd') $this->copyProjectReviewPoint($copyProjectID, $projectID);

        foreach($insertExecutions as $insertExecution)
        {
            $executionID     = $insertExecution->executionID;
            $lastExecutionID = $insertExecutionList[$executionID];
            ...
            {
                if(!isset($insertExecutionList[$insertExecution->parent])) continue;
                $parentID = $insertExecutionList[$insertExecution->parent];
                $pathList = explode(',', $insertExecution->path);
                $path     = ",{$projectID}";
                foreach($pathList as $pathID)
                {
                    if($pathID == $copyProjectID || !isset($insertExecutionList[$pathID])) continue;
                    $path .= "," . $insertExecutionList[$pathID];
                }
            }
            ...
            $this->execution->addExecutionMembers($lastExecutionID, array());

            /* Add the execution team members to parent executions. */
            foreach(explode(',', $path) as $executionParentID)
            {
                if(empty($executionParentID) || $executionParentID == $projectID) continue;
                $this->execution->addExecutionMembers((int)$executionParentID, array($insertExecution->PM));
            }
            ...
            /* 复制任务的模块。 */
            $insertModuleList = $this->copyExecutionTaskModule($executionID, $lastExecutionID);

            /* Add task. */
            $insertTaskList = arrayUnion($insertTaskList, $this->saveTask($copyProjectID, $projectID, $executionID, $lastExecutionID, $insertModuleList, $copyFrom));
        ...
        return array($insertExecutionList, $insertTaskList);
    }
    ...
    public function updateTemplate($projectID, $project, $oldProject)
    {
        $this->dao->update(TABLE_PROJECT)
             ->data($project)->where('id')->eq($projectID)
             ->checkIF(!empty($project->name), 'name', 'unique', "id != $projectID and `type` = 'project' and `deleted` = '0'")
             ->checkIF(!empty($project->code), 'code', 'unique', "id != $projectID and `type` = 'project' and `deleted` = '0'")
             ->autoCheck()->exec();

        if(dao::isError()) return false;

        $this->updateWhitelist($project, $oldProject);
    ...
    {
        $relations = $this->dao->select('*')->from(TABLE_RELATIONOFTASKS)->where('project')->eq($copyProjectID)->fetchAll('id', false);
        if(!empty($relations))
        {
            foreach($relations as $relation)
            {
                $executions = '';
                foreach(explode(',', $relation->execution) as $executionID)
                {
                    if($executionID) $executions .= $executionIdList[$executionID] . ',';
                }

                $relation->project   = $projectID;
                $relation->execution = $executions;
                $relation->pretask   = $taskIdList[$relation->pretask];
                $relation->task      = $taskIdList[$relation->task];
    ...
    {
        $approvals = $this->dao->select('*')->from(TABLE_APPROVALFLOWOBJECT)->where('root')->eq($copyProjectID)->fetchAll('id', false);
        if(!empty($approvals))
        {
            foreach($approvals as $approval)
            {
                unset($approval->id);
                $approval->root = $projectID;
                $this->dao->insert(TABLE_APPROVALFLOWOBJECT)->data($approval)->exec();
            }
        }
    ...
    {
        $taskModules  = $this->dao->select('*')->from(TABLE_MODULE)
        ...
        if(!empty($taskModules))
        {
            ...
            {
                $moduleID       = $module->id;
                $module->root   = $lastExecutionID;
                $module->parent = zget($insertIdList, $module->parent);

                unset($module->id);
                $this->dao->insert(TABLE_MODULE)->data($module)->exec();

                $lastModuleID = $this->dao->lastInsertId();
                $insertIdList[$moduleID] = $lastModuleID;
            }

            $this->loadModel('tree')->fixModulePath($lastExecutionID, 'task');
        }
    ...
    public function copyProjectReviewPoint($copyProjectID, $projectID)
    {
        ...
        if(!empty($points))
        {
            foreach($points as $point)
            {
                ...
                $point->end     = !empty($point->end) ? $point->end : NULL;
                $this->dao->insert(TABLE_OBJECT)->data($point)->exec();
            }
        }
    ...
    public function saveTask($copyProjectID, $projectID, $executionID, $lastExecutionID, $moduleList, $copyFrom = '')
    {
        ...
        if(!empty($tasks))
        {
            $extraFields = $this->loadModel('workflowfield')->getFieldPairs('task', '0');
            foreach($tasks as $task)
            {
                ...
                $insertTask->module      = zget($moduleList, $task->module);
                ...
                $insertTask->keywords    = $task->keywords;
                ...
                if($copyFrom != 'project')
                {
                    foreach($extraFields as $field => $fieldName)
                    {
                        if($field && !empty($task->{$field})) $insertTask->{$field} = $task->{$field};
                    }
                }
                ...
                if(!dao::isError())
                {
                    $this->dao->update(TABLE_TASK)->set('path')->eq(",{$lastTaskID},")->where('id')->eq($lastTaskID)->exec();
    ...
    public function setMenu($projectID)
    {
        ...
        if(!empty($project->isTpl))
        {
            ...
            /* 将甘特图提取到二级导航的任务后面。 */
            if(!empty($this->lang->project->menu->view['subMenu']->gantt))
            {
                $this->lang->project->menu->gantt = $this->lang->project->menu->view['subMenu']->gantt;
            ...
            /* 将QA和过程提取到二级导航的文档后面。 */
            $docOrder = 0;
            foreach($this->lang->project->menuOrder as $order => $menu)
            {
                if($menu == 'doc') $docOrder = $order;
            }
    ...
    public function saveDeliverable($object, $objectType,$postData)
    {
        ...
        /* 文档ID转换成标题。 */
        foreach($newDocs as $index => $docID)
        {
            $newDocs[$index] = isset($docs[$docID]) ? $docs[$docID] : '';
        }

        foreach($delDocs as $index => $docID)
        {
            $delDocs[$index] = isset($docs[$docID]) ? $docs[$docID] : '';
        }
    ...
    */
    ...
    public function getTemplateList($status = 'all', $orderBy = 'id_asc', $pager = null)
    {
        return $this->dao->select('*')->from(TABLE_PROJECT)
            ...
            ->andWhere('(tplAcl')->eq('open')
            ->orWhere("CONCAT(',', `tplWhiteList`, ',')")->like("%,{$this->app->user->account},%")
    ...
    {
        $changes = parent::update($project, $oldProject, $postProductData);

        if($project->workflowGroup == $oldProject->workflowGroup) return $changes;

        /* 如果变更了项目流程，要将项目下的交付物和执行下的交付物合并到新的交付物配置里去。 */
        $projectModel = $project->model;
        if($projectModel == 'agileplus')     $projectModel = 'scrum';
        if($projectModel == 'waterfallplus') $projectModel = 'waterfall';

        $projectType = !empty($oldProject->hasProduct) ? 'product' : 'project';
        $objectCode  = "{$projectType}_{$projectModel}";
        ...
        /* 如果项目曾经提交过交付物。 */
        if(!empty($oldProject->deliverable))
        {
            $oldDeliverable = json_decode($oldProject->deliverable, true);
            foreach($oldDeliverable as $code => $methodDeliverable)
            {
                foreach($methodDeliverable as $method => $deliverables)
                {
                    ...
                    foreach($deliverables as $deliverable)
                    {
                        ...
                        if(!empty($projectDeliverable[$objectCode][$method]))
                        {
                            foreach($projectDeliverable[$objectCode][$method] as $index => $configDeliverable)
                            {
                                if(strpos($deliverable['deliverable'], 'new_') !== false || $configDeliverable['deliverable'] != $deliverable['deliverable']) continue;

                                $projectDeliverable[$objectCode][$method][$index]['doc']  = !empty($deliverable['doc'])  ? $deliverable['doc']  : '';
                                $projectDeliverable[$objectCode][$method][$index]['file'] = !empty($deliverable['file']) ? $deliverable['file'] : '';
                                $duplicated = true;
                            }
                        }
                        ...
                        if(!$duplicated)
                        {
                            $deliverable = array('deliverable' => "new_{$i}", 'required' => false, 'category' => $this->lang->other, 'doc' => zget($deliverable, 'doc', ''), 'file' => zget($deliverable, 'file', ''), 'template' => '');
                            $projectDeliverable[$objectCode][$method][] = $deliverable;
                            $i++;
                        }
                    }
                }
        ...
        $oldProject->deliverable = json_encode($projectDeliverable);
        $this->dao->update(TABLE_PROJECT)->set('`deliverable`')->eq($oldProject->deliverable)->where('id')->eq($oldProject->id)->exec();

        $this->changeExecutionDeliverable($oldProject);
    ...
    {
        $executions = $this->dao->select('*')->from(TABLE_PROJECT)->where('project')->eq($project->id)->fetchAll('id', false);
        foreach($executions as $executionID => $execution)
        {
            ...
            /* 如果迭代曾经提交过交付物。 */
            if(!empty($execution->deliverable))
            {
                $oldDeliverable = json_decode($execution->deliverable, true);
                foreach($oldDeliverable as $code => $methodDeliverable)
                {
                    foreach($methodDeliverable as $method => $deliverables)
                    {
            ...
            $execution->deliverable = json_encode($executionDeliverable);
            $this->dao->update(TABLE_PROJECT)->set('`deliverable`')->eq($execution->deliverable)->where('id')->eq($execution->id)->exec();
        }
