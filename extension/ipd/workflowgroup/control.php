...
class workflowgroup extends control
{
    ...
    *  @param  int    $pageID
    ...
    public function delete($id)
    {
        ...
        $this->dao->update(TABLE_PROJECT)->set('status')->eq('closed')->where('workflowGroup')->eq($id)->andWhere('isTpl')->eq('1')->exec(); // 删除流程时，将项目模板也设置为停用 deleted flow also disabled project template
    ...
    public function deactivate($id)
    {
        ...
        $this->dao->update(TABLE_PROJECT)->set('status')->eq('closed')->where('workflowGroup')->eq($id)->andWhere('isTpl')->eq('1')->exec(); // 停用流程时，将项目模板也设置为停用 disabled flow also disabled project template
