...
class zentaomaxDoc extends docModel
{
    ...
    public function getProductCaseTableCols()
    {
        ...
        if(isset($cols['id']))     $cols['id']['name'] = 'id';
        ...
        foreach($cols as $key => $col)
        {
            $cols[$key]['sortType'] = false;
            if(isset($col['link'])) unset($cols[$key]['link']);
            if($key == 'title') $cols[$key]['link'] = array('url' => helper::createLink('testcase', 'view', "caseID={id}&version={version}"), 'data-toggle' => 'modal', 'data-size' => 'lg');
        }
        return array_values($cols);
    }
    ...
    public function getProductCaseTableData($product, $searchTab, $caseStage = '', $project = array())
    {
        ...
        if(!empty($caseStage)) $cases = array_filter($cases, function($case) use($caseStage) {return strpos(",{$case->stage},", ",{$caseStage},") !== false;});
