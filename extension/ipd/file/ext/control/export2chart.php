...
class myfile extends file
{
    ...
    public function export2Chart()
    {
        ...
        {
            $title = $this->lang->{$this->kind}->report->charts[$item];
            $this->addChartTitle($title, 3);
            if(isset($this->images[$item])) $this->addChartImage($this->images[$item], $item);
            if(isset($this->datas[$item]))  $this->addChartTable($this->datas[$item], $item);
     ...
     * @return void
     */
    public function addCustomTable($tableData, $style = 'block')
    {
        $lineNum = $tableData ? count((array)($tableData[0])) : 3;
        if($lineNum > 8) $lineNum = 8;

        $width       = $lineNum * 800 + 4000;
        ...
        {
            $document .= '<w:tr><w:tblPrEx>' . $tblBorders . $tblLayout . $tblCellMar . '</w:tblPrEx><w:trPr><w:trHeight w:val="287" w:hRule="atLeast"/></w:trPr>';
            foreach($data as $value)
            {
                ...
                $customStyle = '';
                if($rowspan == 0) $customStyle = "<w:vMerge w:val='continue' />";
                if($rowspan > 1)  $customStyle = "<w:vMerge w:val='restart' />";

                $document .= "<w:tc>{$customStyle}<w:p>";
                if($rowspan) $document .= "{$this->wpPr}<w:r>{$this->wrPr}<w:t>{$value}</w:t></w:r>";
                $document .= "</w:p></w:tc>";
            }
    ...
    {
        if(!empty($tableData[0]) && count((array)($tableData[0])) != 3) return $this->addCustomTable($tableData, $style);

        $itemLang    = isset($this->lang->{$this->kind}->report->$item->item) ?  $this->lang->{$this->kind}->report->$item->item : $this->lang->report->item;
        $valueLang   = isset($this->lang->{$this->kind}->report->value) ? $this->lang->{$this->kind}->report->value : $this->lang->report->value;
        $leftWidth   = 3000;
        $centerWidth = 900;
        $rightWidth  = 900;
        ...
        $document .= '<w:tr>
            <w:tblPrEx>' . $tblBorders . $tblLayout . $tblCellMar . '</w:tblPrEx>
            <w:tc>
            <w:tcPr><w:tcW w:w="' . $width . '" w:type="dxa"/><w:gridSpan w:val="3"/></w:tcPr>
            <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . $this->lang->{$this->kind}->report->charts[$item] . '</w:t></w:r></w:p>
            </w:tc>
            </w:tr>';
        $document .= '<w:tr><w:tblPrEx>' . $tblBorders . $tblLayout . $tblCellMar . '</w:tblPrEx>
            ...
            <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . $itemLang . '</w:t></w:r></w:p>
            </w:tc>
            <w:tc>
            <w:tcPr><w:tcW w:w="' . $centerWidth . '" w:type="dxa"/></w:tcPr>
            <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . $valueLang . '</w:t></w:r></w:p>
            </w:tc>
            <w:tc>
            <w:tcPr><w:tcW w:w="' . $rightWidth . '" w:type="dxa"/></w:tcPr>
            <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . $this->lang->report->percent . '</w:t></w:r></w:p>
            </w:tc>
            </w:tr>';
        foreach($tableData as $data)
        {
            $document .= '<w:tr><w:tblPrEx>' . $tblBorders . $tblLayout . $tblCellMar . '</w:tblPrEx>
                ...
                <w:tc>
                <w:tcPr><w:tcW w:w="' . $leftWidth . '" w:type="dxa"/></w:tcPr>
                <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . $data->name . '</w:t></w:r></w:p>
                </w:tc>
                <w:tc>
                <w:tcPr><w:tcW w:w="' . $centerWidth . '" w:type="dxa"/></w:tcPr>
                <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . $data->value . '</w:t></w:r></w:p>
                </w:tc>
                <w:tc>
                <w:tcPr><w:tcW w:w="' . $rightWidth . '" w:type="dxa"/></w:tcPr>
                <w:p>' . $this->wpPr . '<w:r>' . $this->wrPr . '<w:t>' . ($data->percent * 100) . '%' . '</w:t></w:r></w:p>
                </w:tc>
                </w:tr>';
        }
        $document .= '</w:tbl>';
