...
class storyTao extends storyModel
{
     ...
     * @return object|false
     */
    protected function fetchBaseInfo($storyID)
    {
     ...
     * @return array
     */
    protected function getProjectRequirements($productID, $projectID, $pager = null)
    {
     ...
     * @return array
     */
    public function getRelation($storyID, $storyType)
    {
        /* 主动关联。*/
     ...
     * @return array
     */
    protected function getStoriesCountByProductIDs($productIDs, $storyType = 'requirement')
    {
     ...
     * @return array
     */
    protected function getFinishClosedTotal($storyType = 'story')
    {
     ...
     * @return array
     */
    protected function getUnClosedTotal($storyType = 'story')
    {
     ...
     * @return array
     */
    protected function getProductReviewers($productID, $storyReviewers = array())
    {
        $this->loadModel('user');
     ...
     * @return object
     */
    protected function buildStoryTrack($story, $projectID = 0)
    {
     ...
     * @return string
     */
    protected function buildProductsCondition($productIdList, $branch = 'all')
    {
     ...
     * @return array
     */
    protected function mergePlanTitleAndChildren($productID, $stories, $type = 'story')
    {
     ...
     * @return int[]
     */
    protected function extractParents($stories)
    {
     ...
     * @return array
     */
    protected function getChildItems($stories)
    {
        $childItems = array();
     ...
     * @return array
     */
    protected function getExecutionStoriesBySearch($executionID, $queryID, $productID, $orderBy, $storyType = 'story', $sqlCondition = '', $excludeStories = array(), $pager = null)
    {
        /* 获取查询条件。 */
     ...
     * @return string
     */
    protected function replaceAllProductQuery($query)
    {
     ...
     * @return string
     */
    protected function replaceRevertQuery($storyQuery, $productID)
    {
     ...
     * @return array
     */
    protected function getRevertStoryIdList($productID)
    {
     ...
     * @return array
     */
    protected function getModules4ExecutionStories($type, $param)
    {
     ...
     * @return int[]
     */
    protected function fetchExecutionStories($storyDAO, $productID, $type, $branch, $orderBy, $pager = null)
    {
     ...
     * @return int[]
     */
    protected function fetchProjectStories($storyDAO, $productID, $type, $branch, $executionStoryIdList, $orderBy, $pager = null, $project = null)
    {
        if(strpos($orderBy, 'version_') !== false) $orderBy = str_replace('version_', 't2.version_', $orderBy);
     ...
     * @return array
     */
    protected function fixBranchStoryStage($stories)
    {
        if(empty($stories)) return array();
        $rawQuery = $this->dao->get();
     ...
     * @return array
     */
    protected function getUnclosedStatusKeys()
    {
     ...
     * @return array
     */
    protected function getIdListOfExecutionsByProjectID($type, $projectID)
    {
     ...
     * @return array
     */
    protected function getStoriesByProductIdList($productIdList, $storyType = '', $hasParent = true)
    {
     ...
     * @return int|false
     */
    protected function doCreateStory($story)
    {
     ...
     * @return void
     */
    protected function doCreateSpec($storyID, $story, $files = array())
    {
     ...
     * @return object
     */
    protected function doSaveUploadImage($storyID, $fileName, $spec)
    {
     ...
     * @return void
     */
    protected function doCreateReviewer($storyID, $reviewers, $storyVersion = 1)
    {
     ...
     * @return void
     */
    protected function doUpdateSpec($storyID, $story, $oldStory)
    {
     ...
     * @return void
     */
    protected function doChangeParent($storyID, $story, $oldStory)
    {
     ...
     * @return void
     */
    protected function doUpdateLinkStories($storyID, $story, $oldStory)
    {
     ...
     * @return void
     */
    protected function linkToExecutionForCreate($executionID, $storyID, $story, $extra = '')
    {
     ...
     * @return void
     */
    protected function closeBugWhenToStory($bugID, $storyID)
    {
     ...
     * @return void
     */
    protected function finishTodoWhenToStory($todoID, $storyID)
    {
     ...
     * @return bool
     */
    protected function checkCanSubdivide($story, $isShadowProduct)
    {
     ...
     * @return bool
     */
    protected function checkCanSplit($story)
    {
     ...
     * @return array
     */
    protected function getLinkedBranchesAndProjects($storyID)
    {
     ...
     * @return bool
     */
    protected function setStageToPlanned($storyID, $stages = array(), $oldStages = array())
    {
     ...
     * @return bool
     */
    protected function setStageToClosed($storyID, $linkedBranches = array(), $linkedProjects = array())
    {
        $story = $this->dao->findById($storyID)->from(TABLE_STORY)->fetch();
        if(empty($story)) return false;
     ...
     * @return bool
     */
    protected function updateStage($storyID, $stages, $oldStages = array(), $linkedProjects = array())
    {
        $story = $this->dao->findById($storyID)->from(TABLE_STORY)->fetch();
     ...
     * @return void
     */
    public function computeParentStage($story)
    {
        $demandList = array();
     ...
     * @return string
     */
    public function computeStage($children)
    {
        $allWait      = true;
        $hasInRoadmap = false;
        $hasInCharter = false;
     ...
     * @return array
     */
    protected function getDefaultStages($planIdList, $linkedBranches)
    {
     ...
     * @return array
     */
    protected function getLinkedTaskStat($storyID, $linkedProjects)
    {
     ...
     * @return int
     */
    protected function updateLane($storyID, $storyType)
    {
     ...
     * @return int
     */
    protected function updateLinkedLane($storyID, $linkedProjects = array())
    {
     ...
     * @return array
     */
    protected function computeStagesByTasks($storyID, $taskStat = array(), $stages = array(), $linkedProjects = array())
    {
        /* 设置关联的项目的分支阶段为已立项。 */
        if(empty($taskStat))
        {
     ...
     * @return array
     */
    protected function computeStagesByRelease($storyID, $stages)
    {
     ...
     * @return object
     */
    protected function getAffectedProjects($story, $users)
    {
     ...
     * @return object
     */
    protected function getAffectedBugs($story, $users)
    {
     ...
     * @return object
     */
    protected function getAffectedCases($story, $users)
    {
     ...
     * @return object
     */
    protected function getAffectedChildren($story, $users)
    {
     ...
     * @return object
     */
    protected function getAffectedTwins($story, $users)
    {
     ...
     * @return bool
     */
    protected function checkConditions($method, $story)
    {
     ...
     * @return array
     */
    protected function buildBrowseActionBtnList($story, $params = '', $storyType = 'story', $execution = null, $maxGradeGroup = array())
    {
        global $lang;
        $actions = array();
     ...
     * @return bool
     */
    protected function isSuperReviewer()
    {
        $moduleName = $this->app->rawModule;
        return strpos(',' . zget($this->config->{$moduleName}, 'superReviewers', '') . ',', ",{$this->app->user->account},") !== false;
    }
     ...
     * @return object|false
     */
    protected function doUpdateParentStatus($parentID, $status)
    {
     ...
     * @return array
     */
    public function getLeafNodes($stories, $storyType = 'epic')
    {
     ...
     * @return array
     */
    public function buildTrackLanes($leafNodes, $storyType)
    {
        $lanes = array();
     ...
     * @return array
     */
    public function buildTrackCol($name, $title, $parent = 0)
    {
     ...
     * @return array
     */
    public function buildTrackItems($allStories, $leafNodes, $storyType)
    {
     ...
     * @return array
     */
    public function getProjectsForTrack($storyIdList)
    {
     ...
     * @return array
     */
    public function getDesignsForTrack($storyIdList)
    {
     ...
     * @return array
     */
    public function getTasksForTrack($storyIdList)
    {
     ...
     * @return array
     */
    public function mergeChildrenForTrack($allStories, $stories, $storyType)
    {
        $newStories  = array();
        $parentGroup = array();
     ...
     * @return array
     */
    public function reorderStories($stories)
    {
     ...
     * @return array
     */
    public function buildStoryTree(&$stories, $parentId = 0, $originStories = array())
    {
        $tree = array();
     ...
     * @access public
     */
    public function buildReorderResult($parent, &$result)
    {
