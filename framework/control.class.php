...
class control extends baseControl
{
    ...
    {
        if(isset($this->config->{$this->moduleName}) and strpos((string) $this->methodName, 'export') !== false)
        {
     ...
     */
    public function appendExtendFields($fields, $moduleName = '', $methodName = '', $object = null)
    {
     ...
     * @return string
     */
    public function appendExtendCssAndJS($moduleName = '', $methodName = '', $object = null)
    {
        if($this->config->edition == 'open') return '';
     ...
     * @return array
     */
    public function appendExtendForm($position = 'info', $object = null, $moduleName = '', $methodName = '')
    {
        if($this->config->edition == 'open') return array();
        if(!empty($this->app->installing) || !empty($this->app->upgrading)) return array();
     ...
     * @return void
     */
    public function extendRequireFields($objectID = 0)
    {
        if($this->config->edition == 'open') return;
