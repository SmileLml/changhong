...
class productZen extends product
{
     ...
     * @return void
     */
    protected function setProjectMenu($productID, $branch, $preBranch)
    {
     ...
     * @return void
     */
    protected function setCreateMenu($programID)
    {
     ...
     * @return void
     */
    protected function setEditMenu($productID, $programID)
    {
     ...
     * @return void
     */
    protected function setTrackMenu($productID, $branch, $projectID)
    {
        helper::setcookie('preBranch', $branch);
     ...
     * @return void
     */
    protected function setShowErrorNoneMenu($moduleName, $activeMenu, $objectID)
    {
     ...
     * @return void
     */
    private function setShowErrorNoneMenu4QA($activeMenu)
    {
     ...
     * @return void
     */
    private function setShowErrorNoneMenu4Project($activeMenu, $projectID)
    {
     ...
     * @return void
     */
    private function setShowErrorNoneMenu4Execution($activeMenu, $executionID)
    {
     ...
     * @return string
     */
    protected function getBackLink4Create($extra)
    {
     ...
     * @return array
     */
    private function setSelectFormOptions($programID, $fields)
    {
     ...
     * @return array
     */
    protected function getFormFields4Create($programID = 0, $extra = '')
    {
        $extra = str_replace(array(',', ' '), array('&', ''), $extra);
        parse_str($extra, $output);
     ...
     * @return array
     */
    protected function getFormFields4Edit($product)
    {
        /* Init fields. */
     ...
     * @return array
     */
    protected function getFormFields4Close()
    {
        /* Init fields. */
     ...
     * @return array
     */
    protected function getFormFields4Activate()
    {
        /* Init fields. */
     ...
     * @return array
     */
    protected function getProductLines($programIdList = array())
    {
    ...
    protected function getExportFields()
    {
        ...
        {
            if(strpos(strtolower($fieldName), 'epic') !== false && !$this->config->enableER) continue;
            if(strpos(strtolower($fieldName), 'requirement') !== false && !$this->config->URAndSR) continue;
     ...
     * @return array
     */
    protected function getExportData($programID, $browseType, $orderBy, $param = 0, $pager = null)
    {
     ...
     * @return array
     */
    private function getUnauthProgramsOfProducts($products, $authPrograms)
    {
     ...
     * @return array
     */
    protected function getProducts4DropMenu($shadow = '0', $module = '')
    {
     ...
     * @return array
     */
    private function getCreatedLocate($productID, $programID)
    {
        $tab = $this->app->tab;
     ...
     * @return array
     */
    private function getEditedLocate($productID, $programID)
    {
     ...
     * @return void
     */
    protected function buildBatchEditForm($programID, $productIdList)
    {
     ...
     * @return object
     */
    protected function buildProductForCreate($workflowGroup = 0)
    {
     ...
     * @return object
     */
    protected function buildProductForEdit($productID, $workflowGroup = 0)
    {
     ...
     * @return object
     */
    protected function buildProductForActivate($productID)
    {
     ...
     * @return object
     */
    protected function buildProductForClose($productID)
    {
     ...
     * @return array|false
     */
    protected function prepareManageLineExtras($form)
    {
        $data = $form->get();
     ...
     * @return array
     */
    protected function responseAfterCreate($productID, $programID)
    {
     ...
     * @return array
     */
    protected function responseAfterEdit($productID, $programID)
    {
        $message = $this->executeHooks($productID);
        if($message) $this->lang->saveSuccess = $message;
     ...
     * @return array
     */
    protected function responseAfterBatchEdit($programID)
    {
        /* Get location. */
     ...
     * @return object|null
     */
    protected function getBrowseProduct($productID)
    {
     ...
     * @return string
     */
    protected function getBranchID($product, $branch)
    {
     ...
     * @return void
     */
    protected function setMenu4Browse($projectID, $productID, $branch, $storyType)
    {
     ...
     * @param string $browseType
     */
    protected function saveAndModifyCookie4Browse($productID, $branch, $param, $browseType, $orderBy)
    {
     ...
     * @return int
     */
    protected function getModuleId($param, $browseType)
    {
     ...
     * @return array|string
     */
    protected function getModuleTree($projectID, $productID, &$branch, $param, $storyType, $browseType)
    {
     ...
     * @return array
     */
    protected function getProjectProductList($projectID, $isProjectStory)
    {
     ...
     * @return array
     */
    protected function getProductPlans($projectProducts, $projectID, $storyType, $isProjectStory)
    {
     ...
     * @return array
     */
    public function getStories($projectID, $productID, $branchID = '', $moduleID = 0, $param = 0, $storyType = 'all', $browseType = 'allstory', $orderBy = 'id_desc', $pager = null)
    {
     ...
     * @return array
     */
    public function getStoriesByStoryType($productID, $branch = '', $storyType = 'all', $orderBy = 'id_desc', $pager = null)
    {
        /* Append id for second sort. */
        $sort = common::appendOrder($orderBy);
        if(strpos($sort, 'pri_') !== false) $sort = str_replace('pri_', 'priOrder_', $sort);
     ...
     * @return array
     */
    protected function getBranchOptions($projectProducts, $projectID)
    {
     ...
     * @param int $projectID
     */
    protected function getBranchAndTagOption($projectID, $product, $isProjectStory)
    {
        $branchOption    = array();
     ...
     * @return array
     */
    protected function getKanbanList($browseType = 'my')
    {
        $kanbanList = array();
     ...
     * @return void
     */
    protected function saveSession4Browse($product, $browseType)
    {
        $uri = $this->app->getURI(true);
     ...
     * @param int $blockID
     */
    protected function buildSearchFormForBrowse($project, $projectID, &$productID, $branch, $param, $storyType, $browseType, $isProjectStory, $from, $blockID)
    {
     ...
     * @return void
     */
    protected function buildSearchFormForTrack(&$productID, $branch, $projectID, $browseType, $param, $storyType)
    {
        if($this->config->edition == 'ipd' && $storyType == 'story') unset($this->config->product->search['fields']['roadmap']);
        if($this->config->edition == 'ipd' && $storyType == 'requirement')
        {
     ...
     * @return array
     */
    protected function getStoryIdList($stories)
    {
        $storyIdList = array();
     ...
     * @return void
     */
    protected function saveSession4Roadmap()
    {
     ...
     * @return void
     */
    protected function responseNotFound4View()
    {
     ...
     * @return void
     */
    protected function saveBackUriSessionForDynamic()
    {
        $uri = $this->app->getURI(true);
        $this->session->set('productList',     $uri, 'product');
     ...
     * @return [array, object]
     */
    protected function getActionsForDynamic($account, $orderBy, $productID, $type, $date, $direction)
    {
        /* Build parameters. */
     ...
     * @return array
     */
    protected function getActions4Dashboard($productID)
    {
        /* Load pager. */
     ...
     * @return void
     */
    protected function saveBackUriSession4Dashboard()
    {
        $uri = $this->app->getURI(true);
        $this->session->set('productPlanList', $uri, 'product');
        $this->session->set('releaseList',     $uri, 'product');
     ...
     * @return array
     */
    protected function getProductList4Kanban($productList, $planList, $projectList, $releaseList, $projectProduct)
    {
        $kanbanList = array();
     ...
     * @return object
     */
    protected function getEmptyHour()
    {
        $hour = new stdclass();
     ...
     * @return void
     */
    protected function assignBrowseData($stories, $browseType, $storyType, $isProjectStory, $product, $project, $branch, $branchID, $from = '')
    {
     ...
     * @return array
     */
    protected function processProjectListData($projectList)
    {
     ...
     * @return array
     */
    public function getActiveStoryTypeForTrack($projectID = 0, $productID = 0)
    {
