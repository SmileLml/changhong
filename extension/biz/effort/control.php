...
class effort extends control
{
    ...
    public function export($userID, $orderBy = 'id_desc')
    {
        if($_POST)
        {
            ...
            $shadowProducts = $this->product->getPairs('', 0, '', 1);
            ...
            foreach($efforts as $effort)
            {
                ...
                {
                    $effort->project = zget($projects, $effort->project, '');
                    if(empty($effort->project))
                    {
                        $effortProducts  = explode(',', trim($effort->product, ','));
                        foreach($effortProducts as $productID) $effort->project = zget($shadowProducts, $productID, '') . ' ';
                    }
                }
