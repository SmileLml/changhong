...
class model extends baseModel
{
     ...
     * @return object|bool
     */
    public function fetchByID($objectID, $moduleName = '')
    {
    ...
    public function buildFlowMenu($module, $data, $type = 'browse', $show = '')
    {
        ...
        $moduleName = $module;
        if(strpos($module, '.') !== false) [$appName, $moduleName] = explode('.', $module);
        ...
        if($show)
        {
            ...
            {
                if(strpos((string) $action->position, $type) === false || $action->show != $show) continue;
        ...
        else
        {
            ...
            foreach($actions as $action)
            {
                if(strpos((string) $action->position, $type) === false) continue;
     ...
     * @return void
     */
    public function cacheSearchFunc($module, $classMethod, $methodArgs)
    {
     ...
     * @return array
     */
    public function getFlowExtendFields($objectID = 0)
    {
