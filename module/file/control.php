...
class file extends control
{
     ...
     * @return void
     */
    public function buildOldForm($fileCount = 1, $percent = 0.9, $filesName = "files", $labelsName = "labels")
    {
        if(!file_exists($this->file->savePath)) return printf($this->lang->file->errorNotExists, $this->file->savePath);
        if(!is_writable($this->file->savePath)) return printf($this->lang->file->errorCanNotWrite, $this->file->savePath, $this->file->savePath);
     ...
     * @param string $gid
     */
    public function ajaxUpload($uid = '', $objectType = '', $objectID = 0, $extra = '', $field = 'imgFile', $api = false, $gid = '')
    {
     ...
     * @param string $locate
     */
    public function uploadImages($module, $params, $uid = '', $locate = '')
    {
        if($locate)
        {
     ...
     * @param int $stream
     */
    public function read($fileID, $stream = 0)
    {
     ...
     * @return void
     */
    public function ajaxQuery($fileID, $objectType = '', $objectID = 0, $title = '', $extra = '', $stream = 0)
    {
        if(!empty($fileID))
        {
