...
class approvalModel extends model
{
    ...
    public function revert($approvalID, $nodeID, $toNodeID, $revertType = 'order')
    {
        ...
        if($toNodeID == 'start')
        {
            ...
            $reviewers = array($approval->createdBy);
        }
        elseif($toNodeID != 'start')
        {
            ...
            else
            {
                ...
                foreach($oldNodeGroup as $id => $nodes)
                {
                    ...
                    {
                        if($node->prev == $nodeID) break;
                        if($node->node == $toNode->prev) break;
                        ...
                        elseif($node->prev == $toNodeID || ($latestWaitNodeID && ($node->prev == $latestWaitNodeID || $node->node == $latestWaitNodeID)))
                        {
                            $node->status       = 'wait';
                            $node->result       = '';
                            $node->reviewedBy   = '';
                            $node->reviewedDate = null;
                            $latestWaitNodeID = $node->node;
                        }
                    }
                }
            }
            ...
            /* 更新该条数据的审批人。*/
            if($approval->objectType != 'review')
            {
                $table = zget($this->config->objectTables, $approval->objectType, '');
                if(!$table) $table = $this->dao->select('`table`')->from(TABLE_WORKFLOW)->where('module')->eq($approval->objectType)->fetch('table');
                $this->dao->update($table)->set('reviewers')->eq(trim(implode(',', $reviewers), ','))->where('id')->eq($approval->objectID)->exec();
            }

            foreach($oldNodeGroup as $nodes)
            {
                foreach($nodes as $node)
                {
        ...
        $this->loadModel('action')->create($approval->objectType, $approval->objectID, 'Reverted', $this->post->revertOpinion, $nodeID);
    ...
    public function forward($approvalID, $nodeID, $forwardTo, $forwardOpinion)
    {
        ...
        $approval = $this->getByID($approvalID);
        $this->loadModel('action')->create($approval->objectType, $approval->objectID, 'forwarded', $forwardOpinion);
    ...
    private function sendMessage($sendList, $approvalID, $nodeID, $method, $isCC = false)
    {
        ...
        if(isset($messageSetting['message']['setting']))
        {
            ...
            if($isFlow || $isWaterfall)
            {
                ...
                $this->saveNotice($sendList, $objectType, $objectID, $object, $method == 'cancel' ? 'cancel' : $node->node, $method, $isCC);
            }
        }
     ...
     * @return void
     */
    public function saveNotice($sendList, $objectType, $objectID, $object, $node, $action, $isCC)
    {
        $this->loadModel('action');
        ...
        {
            if(in_array($action, array('cancel', 'revert', 'forward')) && isset($this->lang->approval->{$action}))
            {
                $data = $user->realname . $this->lang->approval->{$action} . ':' . html::a($url, "[#{$objectID}::{$title}]");
            }
            else
            {
        ...
        if(in_array($action, array('submit', 'review', 'cancel'))) $action = 'approval' . $action;
        if($action == 'revert' || $action == 'forward') $action = $action . 'ed';
        $actionID = $this->dao->select('id')->from(TABLE_ACTION)
            ->where('objectType')->eq($objectType)
            ->andWhere('objectID')->eq($objectID)
            ->andWhere('actor')->eq($actor)
        ...
        $notify->action      = $actionID ? $actionID : 0;
        $notify->data        = $data;
        $notify->status      = 'wait';
