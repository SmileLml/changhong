...
if($this->config->inQuickon)
{
    ...
    (
        ...
        h::thead(h::th($lang->system->backup->name), h::th($lang->system->backup->status), h::th($lang->system->backup->currentVersion), h::th($lang->system->backup->latestVersion), h::th($lang->actions)),
        h::tbody(setClass('text-center'), h::td($systemInfo->name), h::td($systemInfo->status), h::td(set::title($systemInfo->currentVersionTitle), $systemInfo->currentVersion), h::td
        (
...
else
{
    ...
    foreach($data as $key => $row)
    {
        ...
        foreach($row->files as $file => $attr)
        {
            ...
            if(substr_compare($file, '.php', -strlen('.php')) === 0)        $isPHP = true;
